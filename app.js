...
/* Nombre del archivo en Drive: P01_PA800-SX_antes_03.jpg
   Se calcula al enviar, no al capturar, para que refleje el orden
   y el concepto actuales aunque se hayan editado despues.        */
function slug(t){
  return String(t||'').normalize('NFD').replace(/[\u0300-\u036f]/g,'')
    .replace(/[^A-Za-z0-9]+/g,'-').slice(0,28).replace(/^-+|-+$/g,'') || 'sin-concepto';
}
function nombreFoto(a,m,idx){
  let pre='GEN';
  if(m.partidaId){
    const i=(a.partidas||[]).findIndex(p=>p.id===m.partidaId);
    if(i>=0){
      const p=a.partidas[i];
      pre='P'+String(i+1).padStart(2,'0')+'_'+slug(p.concepto);
    }
  }
  return pre+'_'+m.etapa+'_'+String(idx).padStart(2,'0')+'_'+a.folio+'.jpg';
}
...
async function enviarActa(a,prog){
  if(a.folioProvisional){
    ...
    if(f){
      ...
      /* los nombres de foto se recalculan al subir, no hace falta renombrarlos aqui */
      guardarActa(a);
    }
  }
  ...
  for(let i=0;i<meds.length;i++){
    ...
    const nom=nombreFoto(a,meds[i],i+1);
    meds[i].name=nom; guardarActa(a);
    paq.push({name:nom,mimeType:'image/jpeg',
